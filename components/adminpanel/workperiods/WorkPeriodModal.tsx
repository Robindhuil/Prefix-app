...
export default function WorkPeriodModal({ isOpen, onClose, mode, initialData }: Props) {
    ...
    // Robust scroll lock (runs regardless of render return)
    useEffect(() => {
        if (!isOpen) return;
    ...
    return createPortal(
        <>
            ...
            />
            <div 
                className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[51] p-4"
                onClick={handleBackdropClick}
            >
                <div 
                    className="background rounded-xl shadow-2xl max-w-2xl w-screen max-h-[80vh] overflow-auto"
