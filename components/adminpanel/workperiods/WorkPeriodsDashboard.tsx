...
const PeriodCard = memo(({
    ...
    <div
        className={`
            ...
            ${isActive ? "border-green-600 ring-2 ring-green-500" : "border-decor"}
    ...
    >
        ...
        <div className="pr-10">
            <div className="flex justify-between items-start">
                <h4
                    className={`font-semibold line-clamp-2 pr-8 ${
                        isActive ? "text-green-700 dark:text-green-400" : "text-[#600000]"
                    }`}
                >
                    {period.title}
                    {isActive && (
...
export default function WorkPeriodsDashboard({
    ...
    const isActive = useCallback((s: string, e: string) =>
        new Date(s) <= TODAY && TODAY <= new Date(e), []);
    ...
    return (
        <div
            className={`w-full space-y-4 transition-transform duration-300 ${
                ...
                <div className="space-y-3">
                    {periods.map((period) => (
                        <PeriodCard
                            key={period.id}
                            period={period}
                            isActive={isActive(period.startDate, period.endDate)}
